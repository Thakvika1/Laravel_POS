        ...
        @if ($products->isEmpty())
            <div class="card">
                <div class="card-body" style="text-align:center;padding:48px;color:var(--muted);">
                    ...
                    <a href="{{ route('products.create') }}" class="btn btn-primary" style="margin-top:16px;">+ Add
                        Product</a>
                </div>
            </div>
        @else
                            ...
                            <div class="pc-qty">
                                ...
                                @if ($product->category)
                                    <span class="badge badge-purple"
                                        style="margin-left:4px;">{{ $product->category }}</span>
                                @endif
                            </div>
                        </div>
                        <div class="pc-footer">
...
@push('scripts')
    <script>
        (function() {
            ...
            const buildUrl = () => {
                const url = new URL('{{ route('products.recent') }}', window.location.origin);
